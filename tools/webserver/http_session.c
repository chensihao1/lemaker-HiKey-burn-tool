...
int read_file(char *file_buf)
{
	...
	char *p=strstr(file_buf, dev_name);
	if(p==NULL)
	{
	sprintf(buf, "USB:%s\n", dev_name);
	strcat(file_buf,buf);
	strcat(file_buf,"<div id=\"content2\" style=\"width:500px;height:30px;background:#EDEDED;\">\n");
	sprintf(buf,"<div id=\"pro\" style=\"background:#1E90FF;width:%dpx;height:30px;\"></div>\n", atoi(dev_process+1)*100);
	strcat(file_buf,buf);
	}
    }
